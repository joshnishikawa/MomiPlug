...
void setup(){
  ...
  Bs[6] = new MIDIbutton(tipPin, 107, 0);
...
void loop(){
  ...
  // TODO: 'editing' and 'target' could be reduced to 1 variable inside the sketch
    ...
    if (digitalRead(enableEXP) && Ps[0]->read() == 127){
      editor.editing = true;
      editor.target = 12;
      Ps[0]->mode = !Ps[0]->mode;
  ...
  // In Control Mode ####################################################
    ...
    if (readMIDIthru){
      ...
      USBMIDI.read();
      MIDI.read();
      digitalWrite(led0, chaos(cap0pin, 2000, 2100, 3000, 20, 107) + 1);
